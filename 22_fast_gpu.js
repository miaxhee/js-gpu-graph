...
*/
	...
	var opt = {
		dimensions: [341, 341], //[121, 121], //[85, 85] ,  //[graf_length,graf_length,graf_length],   // ???? define graph.length  ???????????
		mode: 'gpu'    // or cpu
	};


    var myGPUfuncX = gpu.createKernel(function(graf){
	...
	// for (var i = 0; i < graph.length; i++) {
		  ...
		  //for (var j = i + 1; j < graph.length; j++) {
			...
			//ret_node_X += forceX; ret_node_Y += forceY;
			//ret_other_X -= forceX; ret_otherY -= forceY;
	...
	*/
    ...
    var myGPUfuncY = gpu.createKernel(function(graf){
...
*/
  ...
  ret_grafX = myGPUfuncX(graf);    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ret_grafY = myGPUfuncY(graf);    //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      for (var i = 0; i < graph.length; i++){
        for (var j = 0; j < graph.length; j++){
		graf[i][3][0] += ret_grafX[j][i]; graf[i][3][1] += ret_grafY[j][i];  //im Level 3 werden die force Zwischenergebnisse abgespeichert
		graf[j][3][0] -= ret_grafX[j][i]; graf[j][3][1] -= ret_grafY[j][i];
		}
      }
